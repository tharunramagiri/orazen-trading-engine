...
from orazen_client.ft_client import add_arguments, main_exec
...
def get_rest_client():
    client = FtRestClient("http://localhost:8080", "orazenr", "password")
...
def test_FtRestClient_init():
    ...
    assert client._session.auth == ("orazenr", "password")
...
def test_ft_client(mocker, capsys, caplog):
    ...
    mock = mocker.patch("orazen_client.ft_client.FtRestClient._call")
...
def test_ft_client_argparsing(mocker, params, expected_args, expected_kwargs, caplog):
    ...
    mocker.patch("orazen_client.ft_client.load_config", return_value={}, autospec=True)
    mm = mocker.patch(
        f"orazen_client.ft_client.FtRestClient.{mocked_method}", return_value={}, autospec=True
    )
