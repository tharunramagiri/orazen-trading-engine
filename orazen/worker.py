...
from orazen.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from orazen.enums import RPCMessageType, State
from orazen.exceptions import OperationalException, TemporaryError
from orazen.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.orazen = OrazenBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.orazen.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.orazen.notify_status(f"{state.name.lower()}")
            ...
            if state == State.STOPPED:
                self.orazen.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.orazen.strategy.version()
    ...
    def _process_running(self) -> None:
        try:
            ...
            self.orazen.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.orazen.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current orazenbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current orazen modules
        self.orazen.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.orazen.notify_status(f"{State(self.orazen.state)} after config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.orazen:
            self.orazen.notify_status("process died")
            self.orazen.cleanup()
