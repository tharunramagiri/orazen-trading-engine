...
from orazen.configuration import remove_exchange_credentials
from orazen.constants import (
...
from orazen.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from orazen.util import FtTTLCache, PeriodicCache, dt_from_ts, dt_now
from orazen.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Orazen.
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by Orazen.")
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if orazen can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by orazen on this exchange
        """
        ...
        if allow_none_margin_mode and margin_mode is None:
            ...
            if not any(
                ...
                raise ConfigurationError(
                    f"Orazen does not support '{trading_mode}' on {self.name}."
                )
        ...
        if not allow_none_margin_mode and (
            ...
            raise ConfigurationError(
                f"Orazen does not support '{mm_value}' '{trading_mode}' on {self.name}."
            )
    ...
    def fetch_dry_run_order(self, order_id: str) -> CcxtOrder:
        ...
        try:
            ...
            from orazen.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("Orazen does not yet support inverse contracts")
            ...
            raise OperationalException(
                "Orazen only supports isolated futures for leverage trading"
            )
