...
from orazen.constants import ENV_VAR_PREFIX
...
def _flat_vars_to_nested_dict(env_dict: dict[str, Any], prefix: str) -> dict[str, Any]:
    """
    Environment variables must be prefixed with ORAZEN.
    ORAZEN__{section}__{key}
    :param env_dict: Dictionary to validate - usually os.environ
    :param prefix: Prefix to consider (usually ORAZEN__)
...
def environment_vars_to_dict() -> dict[str, Any]:
    ...
    Relevant variables must follow the ORAZEN__{section}__{key} pattern
    :return: Nested dict based on available and relevant variables.
    """
