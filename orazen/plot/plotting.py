...
from orazen.data.history import get_timerange, load_data
...
from orazen.exchange import timeframe_to_prev_date, timeframe_to_seconds
from orazen.misc import pair_to_filename
...
from orazen.util import get_dry_run_wallet
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="Orazen Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "orazen-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="orazen-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
