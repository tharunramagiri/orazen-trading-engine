...
from orazen.enums import RunMode, State
from orazen.exceptions import OperationalException
from orazen.rpc import RPC
from orazen.rpc.api_server.api_pairlists import handleExchangePayload
...
from orazen.rpc.api_server.deps import (
    get_config,
    get_exchange,
    get_rpc,
    get_rpc_optional,
...
def show_config(rpc: RPC | None = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._orazen.state
        strategy_version = rpc._orazen.strategy.version()
...
def get_strategy(
    ...
    if not rpc or config["runmode"] == RunMode.WEBSERVER:
        ...
        # trade mode
        strategy_obj = rpc._orazen.strategy
