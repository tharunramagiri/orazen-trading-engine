...
from orazen.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
...
from orazen.exceptions import ExchangeError, PricingError
from orazen.exchange import Exchange, timeframe_to_minutes, timeframe_to_msecs
from orazen.exchange.exchange_utils import price_to_precision
from orazen.ft_types import AnnotationType
from orazen.loggers import bufferHandler
from orazen.persistence import CustomDataWrapper, KeyValueStore, Order, PairLocks, Trade
from orazen.persistence.models import PairLock, custom_data_rpc_wrapper
from orazen.plugins.pairlist.pairlist_helpers import expand_pairlist
from orazen.rpc.fiat_convert import CryptoToFiatConverter
from orazen.rpc.rpc_types import RPCSendMsg
...
class RPC:
    ...
    def __init__(self, orazen) -> None:
        """
        Initializes all enabled rpc modules
        :param orazen: Instance of a orazen bot
        :return: None
        """
        self._orazen = orazen
        self._config: Config = orazen.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "orazen"),
    ...
    def _rpc_trade_status(self, trade_ids: list[int] | None = None) -> list[dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.is_open:
                    try:
                        current_rate: float = self._orazen.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._orazen.config["stake_currency"],
                        self._orazen.config["fiat_display_currency"],
                    )
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._orazen.wallets.get_total_stake_amount()
        ...
        for trade in trades:
            ...
            if not trade.is_open:
                ...
                if len(trade.select_filled_orders(trade.entry_side)) == 0:
                    ...
                    continue
                try:
                    current_rate = self._orazen.exchange.get_rate(
                        trade.pair, side="exit", is_short=trade.is_short, refresh=False
                    )
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._orazen.wallets.get_starting_balance()
        ...
        current_balance = self._orazen.wallets.get_total_stake_amount()
    ...
    def __balance_get_est_stake(
        ...
        if coin == self._orazen.exchange.get_proxy_coin():
            ...
            try:
                rate: float | None = self._orazen.exchange.get_conversion_rate(
                    coin, stake_currency
                )
                if rate:
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        self._orazen.wallets.update(require_update=False)
        starting_capital = self._orazen.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._orazen.wallets.get_all_balances().items():
            if not balance.total and not balance.free:
                continue

            trade = (
                open_assets.get(coin, None)
                if self._orazen.trading_mode != TradingMode.FUTURES
                else None
            )
            is_stake_currency = coin == self._orazen.exchange.get_proxy_coin()
            is_bot_managed = is_stake_currency or trade is not None
            trade_amount = trade.amount if trade else 0
            if is_stake_currency:
                trade_amount = self._orazen.wallets.get_available_stake_amount()
        ...
        symbol: str
        pos: PositionWallet
        for symbol, pos in self._orazen.wallets.get_all_positions().items():
            est_stake = pos.collateral
            pos_base = self._orazen.exchange.get_pair_base_currency(symbol)
            if pos.leverage and pos.position:
                try:
                    rate = self._orazen.exchange.get_conversion_rate(pos_base, stake_currency)
                    if rate:
        ...
        return {
            ...
            "note": "Simulated balances" if self._orazen.config["dry_run"] else "",
        }

    def _rpc_start(self) -> dict[str, str]:
        """Handler for start"""
        if self._orazen.state == State.RUNNING:
            return {"status": "already running"}

        self._orazen.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> dict[str, str]:
        """Handler for stop"""
        if self._orazen.state != State.STOPPED:
            self._orazen.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        ...
        self._orazen.state = State.RELOAD_CONFIG
    ...
    def _rpc_pause(self) -> dict[str, str]:
        ...
        """
        if self._orazen.state == State.RUNNING:
            self._orazen.state = State.PAUSED
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> dict[str, str]:
        ...
        self._orazen.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = self._orazen.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._orazen.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._orazen.handle_cancel_exit(trade, order, oo, CANCEL_REASON["FORCE_EXIT"])
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            current_rate = (
                self._orazen.exchange.get_rate(
            ...
            order_type = ordertype or self._orazen.strategy.order_types.get(
                "force_exit", self._orazen.strategy.order_types["exit"]
            )
            sub_amount: float | None = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._orazen.exchange.get_min_pair_stake_amount(
            ...
            self._orazen.execute_trade_exit(
                trade,
                current_rate,
                exit_check,
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._orazen.config.get("force_entry_enable", False):
            raise RPCException("Force_entry not enabled.")

        if self._orazen.state != State.RUNNING:
            raise RPCException("trader is not running")

        if order_side == SignalDirection.SHORT and self._orazen.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._orazen.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._orazen.config.get("stake_currency")
        if not self._orazen.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._orazen.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._orazen.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._orazen.strategy.order_types.get(
                "force_entry", self._orazen.strategy.order_types["entry"]
            )
        with self._orazen._exit_lock:
            if self._orazen.execute_entry(
                pair,
                stake_amount,
                price,
                ordertype=order_type,
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._orazen._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._orazen.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
            ...
            if (
                self._orazen.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._orazen.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_count(self) -> dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._orazen.config["max_open_trades"])
                if self._orazen.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> dict:
        ...
        res = {
            "method": self._orazen.pairlists.name_list,
            "length": len(self._orazen.active_pair_whitelist),
            "whitelist": self._orazen.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: list[str]) -> dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._orazen.pairlists.blacklist:
                self._orazen.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: list[str] | None = None) -> dict:
        ...
        if add:
            for pair in add:
                if pair not in self._orazen.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], list(self._orazen.exchange.get_markets().keys()))
                        self._orazen.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._orazen.pairlists.name_list,
            "length": len(self._orazen.pairlists.blacklist),
            "blacklist": self._orazen.pairlists.blacklist,
            "blacklist_expanded": self._orazen.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: int | None) -> dict[str, Any]:
        ...
        #       "orazen.worker", "INFO", "Starting worker develop"]
    ...
    def _rpc_analysed_dataframe(
        ...
        annotations = self._orazen.strategy.ft_plot_annotations(pair=pair, dataframe=_data)

        return RPC._convert_dataframe_to_dict(
            self._orazen.config["strategy"],
            pair,
            timeframe,
            _data,
            last_analyzed,
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._orazen.dataprovider.get_analyzed_dataframe(pair, timeframe)
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._orazen.config["timeframe"]
        candle_type = self._orazen.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _rpc_analysed_history_full(
        ...
        from orazen.data.converter import trim_dataframe
        from orazen.data.dataprovider import DataProvider
        from orazen.persistence.usedb_context import FtNoDBContext
    ...
    def _rpc_plot_config(self) -> dict[str, Any]:
        if (
            self._orazen.strategy.plot_config
            and "subplots" not in self._orazen.strategy.plot_config
        ):
            self._orazen.strategy.plot_config["subplots"] = {}
        return self._orazen.strategy.plot_config
    ...
    def health(self) -> dict[str, str | int | None]:
        last_p = self._orazen.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._orazen.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._orazen.strategy.market_direction
