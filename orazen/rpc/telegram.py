...
from orazen.constants import DUST_PER_COIN, Config
from orazen.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from orazen.exceptions import OperationalException
from orazen.misc import chunks, plural
from orazen.persistence import Trade
from orazen.rpc import RPC, RPCException, RPCHandler
from orazen.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = self._rpc._orazen.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._orazen.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._orazen.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._orazen.state)
