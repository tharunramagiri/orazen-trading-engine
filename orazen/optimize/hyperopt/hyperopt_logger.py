...
def logging_mp_setup(log_queue: Queue, verbosity: int):
    ...
    if current_proc != "MainProcess":
        ...
        root.addHandler(h)
        # Disable orazen logging outside of the main process
        # This only leaves logging from the strategy (unless it's prefixed with "orazen.")
        # and eventually from other libraries.
        if verbosity > logging.DEBUG:
            logging.getLogger("orazen").setLevel(logging.WARNING)
