...
from orazen.configuration import TimeRange, validate_config_consistency
from orazen.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
...
from orazen.data.converter import trim_dataframe, trim_dataframes
from orazen.data.dataprovider import DataProvider
from orazen.data.metrics import combined_dataframes_with_rel_mean
...
from orazen.exchange.exchange import TICK_SIZE, Exchange
from orazen.ft_types import (
    BacktestContentType,
...
from orazen.optimize.backtest_caching import get_strategy_run_id
from orazen.optimize.optimize_reports import (
...
from orazen.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from orazen.util import FtPrecise, dt_now, get_progress_tracker
from orazen.util.migrations import migrate_data
...
class Backtesting:
    ...
    def __init__(
        ...
        *,
        ...
        if self.config.get("freqai_backtest_live_models", False):
            from orazen.freqai.utils import get_timerange_backtest_live_models
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # orazen does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_entry_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which orazen does not support in live.
