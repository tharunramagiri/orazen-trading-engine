"""
Orazen is the main module of this bot. It contains the OrazenBot class.
"""
...
from orazen.configuration import remove_exchange_credentials, validate_config_consistency
from orazen.constants import BuySell, Config, EntryExecuteMode, ExchangeConfig, LongShort
from orazen.data.converter import order_book_to_dataframe
...
from orazen.exchange.exchange_types import CcxtOrder
from orazen.leverage.liquidation_price import update_liquidation_prices
from orazen.misc import safe_value_fallback, safe_value_fallback2
from orazen.mixins import LoggingMixin
from orazen.persistence import Order, PairLocks, Trade, init_db
...
from orazen.plugins.protectionmanager import ProtectionManager
from orazen.resolvers import ExchangeResolver, StrategyResolver
from orazen.rpc import RPCManager
from orazen.rpc.external_message_consumer import ExternalMessageConsumer
from orazen.rpc.rpc_types import (
    ProfitLossStr,
    RPCCancelMsg,
    RPCEntryMsg,
    RPCExitCancelMsg,
...
from orazen.util import FtPrecise, MeasureTime, PeriodicCache, dt_from_ts, dt_now
from orazen.util.migrations import migrate_live_content
from orazen.wallets import Wallets


logger = logging.getLogger(__name__)


class OrazenBot(LoggingMixin):
    """
    Orazen is the main class of the bot.
    ...
    def __init__(self, config: Config) -> None:
        ...
        try:
            ...
            # initialization, even before Orazenbot has a chance to start its throttling,
            # so anything in the Orazenbot instance should be ready (initialized), including
    ...
    def process(self) -> None:
        ...
        # Without this, orazen may try to recreate stoploss_on_exchange orders
    ...
    def handle_onexchange_order(self, trade: Trade) -> bool:
        ...
        try:
            ...
            # catching https://github.com/orazen/orazen/issues/9025
            Trade.rollback()
