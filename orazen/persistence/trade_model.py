...
from orazen.misc import safe_value_fallback
from orazen.persistence.base import ModelBase, SessionType
from orazen.persistence.custom_data import CustomDataWrapper, _CustomData
from orazen.util import FtPrecise, dt_from_ts, dt_now, dt_ts, dt_ts_none, round_value
...
class LocalTrade:
    ...
    def calc_close_trade_value(self, rate: float, amount: float | None = None) -> float:
        ...
        if trading_mode == TradingMode.SPOT:
            ...
            raise OperationalException(
                f"{self.trading_mode} trading is not yet available using orazen"
            )
