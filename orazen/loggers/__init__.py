...
from orazen.loggers.buffering_handler import FTBufferingHandler
from orazen.loggers.ft_rich_handler import FtRichHandler
from orazen.loggers.rich_console import get_rich_console


# from orazen.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "orazen": logging.INFO if verbosity < 1 else logging.DEBUG,
        "orazen.exchange.exchange_ws": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "orazen.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running orazen using docker, you see this error "
