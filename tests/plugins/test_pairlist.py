...
from orazen.constants import AVAILABLE_PAIRLISTS
...
from orazen.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from orazen.plugins.pairlistmanager import PairListManager
from orazen.resolvers import PairListResolver
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    orazen = get_patched_orazenbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = orazen.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(orazen.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", orazen.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    orazen.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(orazen.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == orazen.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(orazen.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == orazen.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(orazen.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == orazen.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = orazen.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    orazen = get_patched_orazenbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_orazenbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    orazen.pairlists.refresh_pairlist()
    assert whitelist == orazen.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(orazen.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "orazen.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    orazen = get_patched_orazenbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_orazenbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "orazen.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        orazen.pairlists.refresh_pairlist()
        whitelist = orazen.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_orazenbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert orazen.pairlists.name_list == [pairlist]
    assert pairlist in str(orazen.pairlists.short_desc())
    assert isinstance(orazen.pairlists.whitelist, list)
    assert isinstance(orazen.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with Orazen"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = orazen.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_no_range(mocker, whitelist_conf, lookback_conf, caplog):
    ...
    with pytest.raises(
        ...
        get_patched_orazenbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    orazen = get_patched_orazenbot(mocker, whitelist_conf)
    assert len(orazen.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    orazen.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(orazen.pairlists._pairlist_handlers[0]._pair_cache) == 1
    orazen.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        orazen = get_patched_orazenbot(mocker, whitelist_conf_agefilter)
        assert orazen.exchange.refresh_latest_ohlcv.call_count == 0
        orazen.pairlists.refresh_pairlist()
        assert len(orazen.pairlists.whitelist) == 3
        assert orazen.exchange.refresh_latest_ohlcv.call_count > 0

        orazen.pairlists.refresh_pairlist()
        assert len(orazen.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert orazen.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        orazen.pairlists.refresh_pairlist()
        assert len(orazen.pairlists.whitelist) == 3
        assert orazen.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        orazen.pairlists.refresh_pairlist()
        assert len(orazen.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert orazen.exchange.refresh_latest_ohlcv.call_count == 0
    orazen.pairlists.refresh_pairlist()
    assert len(orazen.pairlists.whitelist) == expected_length
    assert orazen.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = orazen.exchange.refresh_latest_ohlcv.call_count
    orazen.pairlists.refresh_pairlist()
    assert len(orazen.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert orazen.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_range_volatility_filter_lookback_timeframe(
    ...
    orazen = get_patched_orazenbot(mocker, default_conf)
    orazen.pairlists.refresh_pairlist()
    assert len(orazen.pairlists.whitelist) == 5
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        orazen = get_patched_orazenbot(mocker, whitelist_conf)
        short_desc = str(orazen.pairlists.short_desc())
...
def test_performance_filter(
    ...
    orazen = get_patched_orazenbot(mocker, allowlist_conf)
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "orazen.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_categories_list",
        return_value=[
            {"category_id": "layer-1"},
    ...
    gcm_mock = mocker.patch(
        "orazen.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
...
def test_CrossMarketPairList_breaks_prefix_loop_on_match(mocker, default_conf_usdt, markets):
    ...
    mocker.patch(
        "orazen.plugins.pairlist.CrossMarketPairList.PairPrefixes",
