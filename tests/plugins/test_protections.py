...
from tests.conftest import get_patched_orazenbot, log_has_re
...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in orazen.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog, is_short):
    ...
    assert not orazen.protections.global_stop()
    ...
    assert orazen.protections.global_stop()
    ...
    orazen.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair, only_per_side):
    ...
    message = r"Trading stopped due to .*"
    pair = "XRP/BTC"
    assert not orazen.protections.stop_per_pair(pair)
    assert not orazen.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not orazen.protections.stop_per_pair(pair)
    assert orazen.protections.global_stop() != only_per_pair
    ...
    orazen.protections.stop_per_pair(pair)
    assert orazen.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not orazen.protections.global_stop()
    assert not orazen.protections.stop_per_pair("XRP/BTC")
    ...
    assert not orazen.protections.global_stop()
    assert orazen.protections.stop_per_pair("XRP/BTC")
    ...
    assert orazen.protections.stop_per_pair("ETH/BTC")
...
def test_LowProfitPairs(mocker, default_conf, fee, caplog, only_per_side):
    ...
    Trade.commit()
    assert orazen.protections.stop_per_pair("XRP/BTC") != only_per_side
    ...
    assert orazen.protections.global_stop() != only_per_side
    assert orazen.protections.stop_per_pair("XRP/BTC") != only_per_side
...
def test_MaxDrawdown_ratio_mode(mocker, default_conf, fee, caplog):
    ...
    assert not orazen.protections.global_stop(starting_balance=starting_balance)
    assert not orazen.protections.stop_per_pair("XRP/BTC", starting_balance=starting_balance)
    caplog.clear()
    ...
    assert not PairLocks.is_pair_locked("XRP/BTC")
    assert orazen.protections.global_stop(starting_balance=starting_balance)
    assert PairLocks.is_global_lock()
    assert log_has_re(message, caplog)
...
def test_MaxDrawdown_mode_comparison(mocker, default_conf, fee, calculation_mode, expected_locked):
    ...
    lock = orazen.protections.global_stop(starting_balance=starting_balance)
...
def test_MaxDrawdown_threshold_boundary(mocker, default_conf, calculation_mode):
    ...
    orazen = get_patched_orazenbot(mocker, default_conf)
    handler = next(p for p in orazen.protections._protection_handlers if p.name == "MaxDrawdown")
...
def test_protection_manager_desc(
    ...
    short_desc = str(orazen.protections.short_desc())
