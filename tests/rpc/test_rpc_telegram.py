...
from orazen.constants import CANCEL_REASON
...
from orazen.loggers import setup_logging
from orazen.persistence import PairLocks, Trade
from orazen.persistence.models import Order
from orazen.rpc import RPC
from orazen.rpc.rpc import RPCException
from orazen.rpc.telegram import Telegram, authorized_only
...
def mock_exchange_loop(mocker):
    mocker.patch("orazen.exchange.exchange.Exchange._init_async_loop")
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None, mock_fiat=True):
    ...
    if mock:
        mocker.patch.multiple(
            "orazen.rpc.telegram.Telegram",
            _init=MagicMock(),
            _send_msg=msg_mock,
            _start_thread=MagicMock(),
        )
    if not ftbot:
        ftbot = get_patched_orazenbot(mocker, default_conf)
    rpc = RPC(ftbot)
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch("orazen.rpc.telegram.Telegram._init", MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    app_mock = MagicMock()
    mocker.patch("orazen.rpc.telegram.Telegram._start_thread", MagicMock())
    mocker.patch("orazen.rpc.telegram.Telegram._init_telegram_app", return_value=app_mock)
    mocker.patch("orazen.rpc.telegram.Telegram._startup_telegram", AsyncMock())
...
async def test_telegram_startup(default_conf, mocker, caplog) -> None:
    ...
    sleep_mock = mocker.patch("orazen.rpc.telegram.asyncio.sleep", AsyncMock())
...
async def test_telegram_cleanup(
    ...
    # mocker.patch('orazen.rpc.telegram.Application', app_mock)
...
async def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch("orazen.rpc.telegram.Telegram._status_table", status_table)
...
async def test_order_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, orazenbot, msg_mock = get_telegram_testobject(mocker, default_conf)
    ...
    mocker.patch("orazen.rpc.telegram.MAX_MESSAGE_LENGTH", 500)
    ...
    msg_mock.reset_mock()
    mocker.patch("orazen.rpc.telegram.MAX_MESSAGE_LENGTH", 50)
    context = MagicMock()
    context.args = ["2"]
    await telegram._order(update=update, context=context)
...
async def test_daily_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    ...
    telegram, _orazenbot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_weekly_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    ...
    telegram, orazenbot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_telegram_profit_handle(
    ...
    mocker.patch("orazen.wallets.Wallets.get_starting_balance", return_value=1000)
    assert (
...
async def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    ...
    orazenbot.config["dry_run"] = False
    await telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
async def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch(
        "orazen.rpc.rpc.RPC._rpc_balance",
        return_value={
            "currencies": balances,
            "total": 100.0,
            "total_bot": 100.0,
...
async def test_pause_handle(default_conf, update, mocker) -> None:
    ...
    assert orazenbot.state == State.PAUSED
    assert msg_mock.call_count == 1
    assert (
...
async def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert orazenbot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
async def test_telegram_forceexit_handle(
    ...
    msg_mock = mocker.patch("orazen.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("orazen.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
    ...
    orazenbot = OrazenBot(default_conf)
    rpc = RPC(orazenbot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(orazenbot)
...
async def test_force_exit_no_pair(default_conf, update, ticker, fee, mocker) -> None:
    ...
    femock = mocker.patch("orazen.rpc.rpc.RPC._rpc_force_exit")
    telegram, orazenbot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_force_enter_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch("orazen.rpc.rpc.RPC._rpc_force_entry", fbuy_mock)

    telegram, orazenbot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(orazenbot)
...
async def test_force_enter_no_pair(default_conf, update, mocker) -> None:
    fbuy_mock = mocker.patch("orazen.rpc.rpc.RPC._rpc_force_entry", return_value=None)
...
async def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "orazen.rpc.rpc.RPC._rpc_enter_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_exit_reason_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "orazen.rpc.rpc.RPC._rpc_exit_reason_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_mix_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "orazen.rpc.rpc.RPC._rpc_mix_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_whitelist_static(default_conf, update, mocker) -> None:
    telegram, _orazenbot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert orazenbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert orazenbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
async def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "orazen\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch("orazen.rpc.telegram.MAX_MESSAGE_LENGTH", 200)
    context = MagicMock()
    context.args = []
    await telegram._logs(update=update, context=context)
...
async def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    orazenbot.strategy.version = lambda: "1.1.1"
...
async def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    orazenbot.config["trailing_stop"] = True
    await telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert "*Mode:* `{}`".format("Dry-run") in msg_mock.call_args_list[0][0][0]
    assert "*Exchange:* `binance`" in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_enter_notification(
    ...
    mocker.patch(
        "orazen.data.dataprovider.DataProvider.get_analyzed_dataframe", return_value=(df, 1)
    )
    ...
    telegram, orazenbot, msg_mock = get_telegram_testobject(
    ...
    orazenbot.config["telegram"]["notification_settings"] = {"entry": "off"}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    orazenbot.config["telegram"]["notification_settings"] = {"entry": "silent"}
    caplog.clear()
    msg_mock.reset_mock()
...
async def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    orazenbot.config["telegram"]["enabled"] = True
    telegram = init_telegram(orazenbot)
    await telegram._send_msg("test")
    ...
    orazenbot.config["telegram"]["enabled"] = True
    orazenbot.config["telegram"]["keyboard"] = invalid_keys_list
    err_msg = (
        re.escape(
    ...
    orazenbot.config["telegram"]["enabled"] = True
    orazenbot.config["telegram"]["keyboard"] = custom_keys_list
    telegram = init_telegram(orazenbot)
    await telegram._send_msg("test")
    used_keyboard = bot.send_message.call_args[1]["reply_markup"]
...
async def test_change_market_direction(default_conf, mocker, update) -> None:
    ...
    assert telegram._rpc._orazen.strategy.market_direction == MarketDirection.NONE
    context = MagicMock()
    context.args = ["long"]
    await telegram._changemarketdir(update, context)
    assert telegram._rpc._orazen.strategy.market_direction == MarketDirection.LONG
    context = MagicMock()
