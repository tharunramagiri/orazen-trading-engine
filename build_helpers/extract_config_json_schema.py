...
def extract_config_json_schema():
    try:
        ...
        from orazen.config_schema import CONF_SCHEMA
    except ImportError:
        # If orazen is not installed, add the parent directory to sys.path
        # to import directly from the source
        import sys

        script_dir = Path(__file__).parent
        orazen_dir = script_dir.parent
        sys.path.insert(0, str(orazen_dir))
